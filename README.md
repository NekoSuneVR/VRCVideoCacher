...
| AutoInstallCodecs         | When `true`, the client tries to install missing Windows media codecs with `winget`. Default is `false` to avoid startup noise or source issues on systems where the Microsoft Store source is broken.                                                                                       |
